      ...
      expect(
        ...
        "main" AS (
          SELECT
            ...
            AVG("session_length") AS "Avg. Session length"
          FROM "common" AS "t"
          WHERE TIME_SHIFT(TIMESTAMP '2019-08-26 00:00:00', 'PT1H', -1) <= "__time" AND "__time" < TIMESTAMP '2019-08-26 00:00:00'
          GROUP BY 1
          ORDER BY "Count" DESC
          LIMIT 200
        ),
        "compare_P1D" AS (
          ...
          WHERE TIME_SHIFT(TIME_SHIFT(TIMESTAMP '2019-08-26 00:00:00', 'PT1H', -1), 'P1D', -1) <= "__time" AND "__time" < TIME_SHIFT(TIMESTAMP '2019-08-26 00:00:00', 'P1D', -1)
          GROUP BY 1
        )
        SELECT
